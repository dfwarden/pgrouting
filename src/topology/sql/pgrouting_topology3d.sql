CREATE OR REPLACE FUNCTION pgr_createTopology3D(edge_table text, tolerance double precision, 
...
BEGIN
    msgKind = 1; -- notice
    fnName = 'pgr_createToplogy3D';
    raise notice 'PROCESSING:'; 
    raise notice 'pgr_createToplogy3D(''%'', %, ''%'', ''%'', ''%'', ''%'', rows_where := ''%'', clean := %)',edge_table,tolerance,the_geom,id,source,target,rows_where, clean;
    ...
    raise notice 'Creating Topology, Please wait...';
		raise NOTICE 'going to run query to get table of start and end points for each thing';
        rowcount := 0;
        ...
        LOOP
            ...
            source_id := _pgr_pointToId3D(points.source, tolerance,vertname,srid);
            target_id := _pgr_pointToId3D(points.target, tolerance,vertname,srid);
            BEGIN                         
...
COMMENT ON FUNCTION pgr_createTopology3D(text, double precision,text,text,text,text,text,boolean) 
